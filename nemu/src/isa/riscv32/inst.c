...
#define Jal() do { R(rd) = s->pc + 4; s->dnpc += imm - 4; } while(0)    // why?
#define Jalr() do { word_t t = s -> pc; s->dnpc = (src1 + imm) &~ 1; R(rd) = t; } while(0)
#define Bne() do { if (src1 != src2) s->dnpc += imm; } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = BITS(BITS(src1, 30, 0) << src2, 30, 0));
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", slti   , R, R(rd) = ((int32_t)src1 < (int32_t)imm) ? 1 , 0);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , R, R(rd) = (src1 < imm) ? 1 , 0);

  /* Load and Store Instructions */
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));   // store word
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, Mr(src1 + imm, 4, R(rd));
