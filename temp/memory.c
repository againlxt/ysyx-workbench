...
uint8_t* guest_to_host_sram(int32_t addr) { return sram + addr; }
uint8_t* guest_to_host_psram(int32_t addr) { return psram + addr; }

static word_t sram_host_read(void* addr, int len) {
	switch (len) {
		case 0: return 0;
...
static void sram_host_write(void *addr, int len, word_t data) {
	switch (len) {
		case 0: return;
...
int unsigned pmem_read(unsigned int addr, unsigned char wmask)
{
	if (likely(in_pmem(addr))) {
		word_t ret = sram_host_read(guest_to_host_sram(addr), 4);
		#ifdef CONFIG_MTRACE
...
extern "C" void sram_write(unsigned int waddr, unsigned int wdata, unsigned char wmask);
static unsigned int pre_waddr = 0;
static unsigned int pre_wdata = 0;
static unsigned int pre_wmask = 0;

void sram_write(unsigned int waddr, unsigned int wdata, unsigned char wmask) {
	...
	if (likely(in_pmem(waddr))) {
		...
		sram_host_write(guest_to_host_sram(waddr), len, data);
		 #ifdef CONFIG_MTRACE
